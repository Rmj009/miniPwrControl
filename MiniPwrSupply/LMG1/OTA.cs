    ...
    public partial class LMG1_OTA
    {
        ...
        private void JudgeFW_Jason_Modfied()
        {
            ...
            try
            {
                ...
                string BLEFW_Upgrade_Cmd = Func.ReadINI("Setting", "BLE", "BLEFW_Upgrade_Cmd", "bt_upgrade_utility -f /lib/firmware/efr32/rcp4.3.0_no_encrypt_afh.gbl -p /dev/ttyMSM1");
        ...
        private void OTA_Thread()
        {
            ...
            private void Start_Thread_Scan(string broadcast_channel, string broadcast_pan_id)
            {
                ...
                try
                {
                    ...
                    if (!ChkInitial(PortType.GOLDEN_SSH, "root@Bridge_golden", 120 * 1000))
                    {
                ...
                RetrygetRSSI: //Jason add RETRY to solve RSSI Check NG 2023/10/15
                    ...
                    for (int i = 1; i <= rssiCount; i++)
                    {
                        ...
                        SendAndChk(PortType.GOLDEN_SSH, $"ot-ctl scan {broadcast_channel}", "root@Bridge_golden", out res, 0, 10 * 1000);
                        if (res.Contains("connect session failed: No such file or directory"))
                        {
                            //Enable CPCD
                            SendAndChk(PortType.GOLDEN_SSH, "service cpcd start", keyword, out res, 0, 6000);
                            DisplayMsg(LogType.Log, "[Golden] Delay 10s...");
                            Thread.Sleep(10000); //一定要delay,不然下一步會error

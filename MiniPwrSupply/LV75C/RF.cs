...
namespace MiniPwrSupply.LV75C
{
    public partial class RF
    {
        ...
        private void RF_Test()
        {
            try
            {
                ...
                //else if (Func.ReadINI("Setting", "Port", "RelayBoard", "Disable").ToUpper() == "ENABLE")
                {
                    SwitchRelay(CTRL.ON);
                    int timeDelay = 5;
                    int.TryParse(Func.ReadINI("Setting", "DelayTime", "SwitchTime", "10"), out timeDelay);
                    DisplayMsg(LogType.Log, $"PowerOFF Delay {timeDelay}");
                ...
                this.ChkLV75CBootUp(infor);                //ChkBootUp(PortType.SSH);
                                                           //if (isLoop == 0)
                                                           //    CheckEthernetMAC();
        ...
        private void RF_WiFi()
        {
            ...
            string IP_dut1st = Func.ReadINI("Setting", "IP", "Dut_1_IP", "192.168.1.11");
            string IP_dut2nd = Func.ReadINI("Setting", "IP", "Dut_1_IP", "192.168.1.12");
            string IP_dut3rd = Func.ReadINI("Setting", "IP", "Dut_1_IP", "192.168.1.20");
            string IP_dut4th = Func.ReadINI("Setting", "IP", "Dut_1_IP", "192.168.1.60");
            string IP_dut5th = Func.ReadINI("Setting", "IP", "Dut_1_IP", "192.168.1.50");
            List<string> dutNth = new List<string> { "Dut1", "Dut2", "Dut3", "Dut4", "Dut5" };
            List<string> dutIP_Nth = new List<string> { IP_dut1st, IP_dut2nd, IP_dut3rd, IP_dut4th, IP_dut5th };
            try
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    IsPass = this.chkViaDutPing(dutIP_Nth[i], dutNth[i]);
                    if (IsPass == false)
                    {
                        break;
                    }
                ...
                //IsPass = this.chkViaDutPing(IP_dut1st, dutNth[0]);
                //IsPass = this.chkViaDutPing(IP_dut2nd);
                //IsPass = this.WiFi_GoldenSetting(WiFiType.WiFi_2G);

                //IsPass = this.chkViaDutPing(IP_dut3rd); // 2G
                //IsPass = this.chkViaDutPing(IP_dut5th); // 6G
                //IsPass = this.chkViaDutPing(IP_dut4th); // 5G
                //EnterWiFiTestMode();
        ...
        private void EnterWiFiTestMode()
        {
            ...
            try
            {
                //while (showRetry++ < 5)
                //{
                //    SendAndChk(PortType.SSH, "uci show wireless", keyword, out res, 1000, 30 * 1000);
                //    string[] reslines = res.Split('\n');
                //    Thread.Sleep(2000);
                //    if (reslines.Length >=48)
                //    {
                //        DisplayMsg(LogType.Log, "OKKKK");
                //        break;
                //    }
                //}
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    {
                        DisplayMsg(LogType.Log, @"rmmod OK");
                        break;
                    }
                    SendAndChk(PortType.SSH, sCtrlC, keyword, out res, 0, 1000);
                    if (i > 5)
                    {
                        DisplayMsg(LogType.Log, @"rmmod NG");
                        MessageBox.Show("DEBUG rmmod");
                    }
        ...
        private bool chkViaDutPing(string dut_ip, string dutNth)
        {
            bool IsPass = false;
            if (!CheckGoNoGo())
            {
            ...
            string item = "DutPing";
            //string keyword = "root@LV75C:/#";
            string keyword = "packet loss";
            string res = "";

            //string IP_dut1st = "192.168.1.11";
            //string IP_dut2nd = "192.168.1.12";
            int delayMs = 500;
            int timeout = 1000;
            if (dutNth == "Dut5" || dutNth == "Dut4" || dutNth == "Dut3")
            {
                delayMs = 5000;
                timeout = 5000;
            }
            ...
            try
            {
                DisplayMsg(LogType.Log, $"=============== {item} {dutNth}===============");
                for (int i = 0; i < 6; i++)
                {
                    if (SendAndChk(PortType.UART, $"ping {dut_ip} -c 1", keyword, out res, delayMs, timeout))
                    {
                        if (res.Contains("0% packet loss"))
                        {
                            IsPass = true;
                            break;
                        }
